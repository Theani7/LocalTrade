...
- **Fix Empty / List Condition**:
  - Replace the current broken `if (provider.users.isEmpty)` and nested `provider.products.isEmpty` logic with a proper state check:
    ...
    - Else:
      - Render the user list separated with dividers.
...
### 3.4 Reusable Active Search Banner Widget
...
2. **Tab Verification**:
   - Verify searching for non-existent users (e.g. "abc"):
     - Shows `No customers found` empty state with clear query message and working `Clear search` button.
     - Tapping `Clear search` clears the field and re-loads users.
   - Verify searching for existing users:
     - Shows matching users + active search query banner with result count + functional `Clear` button.
   - Repeat verification for Vendors tab (including status filter combinations).
   - Repeat verification for Products tab.
